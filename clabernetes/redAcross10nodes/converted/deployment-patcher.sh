...
PATCH=$(kubectl get deployment rupf -n $NS -o yaml | yq '.spec.template.metadata.annotations."k8s.v1.cni.cncf.io/networks" = "[{\"name\": \"net2001\", \"namespace\": \"across-tc32\", \"interface\": \"net2001\"},{\"name\": \"net2002\", \"namespace\": \"across-tc32\", \"interface\": \"net2002\"}]"' -); kubectl patch deployment rupf -n $NS -o yaml --patch "$PATCH"
...
PATCH=$(kubectl get deployment r11 -n $NS -o yaml | yq '.spec.template.metadata.annotations."k8s.v1.cni.cncf.io/networks" = "[{\"name\": \"net2005\", \"namespace\": \"across-tc32\", \"interface\": \"net2005\"},{\"name\": \"net1001\", \"namespace\": \"across-tc32\", \"interface\": \"net1001\"},{\"name\": \"net1005\", \"namespace\": \"across-tc32\", \"interface\": \"net1005\"}]"' -); kubectl patch deployment r11 -n $NS -o yaml --patch "$PATCH"
...
PATCH=$(kubectl get deployment r13 -n $NS -o yaml | yq '.spec.template.metadata.annotations."k8s.v1.cni.cncf.io/networks" = "[{\"name\": \"net2004\", \"namespace\": \"across-tc32\", \"interface\": \"net2004\"},{\"name\": \"net1004\", \"namespace\": \"across-tc32\", \"interface\": \"net1004\"},{\"name\": \"net1008\", \"namespace\": \"across-tc32\", \"interface\": \"net1008\"}]"' -); kubectl patch deployment r13 -n $NS -o yaml --patch "$PATCH"
...
PATCH=$(kubectl get deployment r12 -n $NS -o yaml | yq '.spec.template.metadata.annotations."k8s.v1.cni.cncf.io/networks" = "[{\"name\": \"net2006\", \"namespace\": \"across-tc32\", \"interface\": \"net2006\"},{\"name\": \"net1013\", \"namespace\": \"across-tc32\", \"interface\": \"net1013\"},{\"name\": \"net1014\", \"namespace\": \"across-tc32\", \"interface\": \"net1014\"}]"' -); kubectl patch deployment r12 -n $NS -o yaml --patch "$PATCH"
...
PATCH=$(kubectl get cm across -n $NS -o yaml | yq '.data.r11 |= (. | fromjson | .topology.links += [{"endpoints": ["macvlan:net2005", "r11:eth1"]}] | tojson)'); kubectl patch cm across -n $NS -o yaml --patch "$PATCH"
PATCH=$(kubectl get cm across -n $NS -o yaml | yq '.data.r11 |= (. | fromjson | .topology.links += [{"endpoints": ["macvlan:net1001", "r11:eth2"]}] | tojson)'); kubectl patch cm across -n $NS -o yaml --patch "$PATCH"
PATCH=$(kubectl get cm across -n $NS -o yaml | yq '.data.r11 |= (. | fromjson | .topology.links += [{"endpoints": ["macvlan:net1005", "r11:eth3"]}] | tojson)'); kubectl patch cm across -n $NS -o yaml --patch "$PATCH"
PATCH=$(kubectl get cm across -n $NS -o yaml | yq '.data.r12 |= (. | fromjson | .topology.links += [{"endpoints": ["macvlan:net2006", "r12:eth1"]}] | tojson)'); kubectl patch cm across -n $NS -o yaml --patch "$PATCH"
PATCH=$(kubectl get cm across -n $NS -o yaml | yq '.data.r12 |= (. | fromjson | .topology.links += [{"endpoints": ["macvlan:net1013", "r12:eth2"]}] | tojson)'); kubectl patch cm across -n $NS -o yaml --patch "$PATCH"
PATCH=$(kubectl get cm across -n $NS -o yaml | yq '.data.r12 |= (. | fromjson | .topology.links += [{"endpoints": ["macvlan:net1014", "r12:eth3"]}] | tojson)'); kubectl patch cm across -n $NS -o yaml --patch "$PATCH"
PATCH=$(kubectl get cm across -n $NS -o yaml | yq '.data.r13 |= (. | fromjson | .topology.links += [{"endpoints": ["macvlan:net2004", "r13:eth1"]}] | tojson)'); kubectl patch cm across -n $NS -o yaml --patch "$PATCH"
PATCH=$(kubectl get cm across -n $NS -o yaml | yq '.data.r13 |= (. | fromjson | .topology.links += [{"endpoints": ["macvlan:net1004", "r13:eth2"]}] | tojson)'); kubectl patch cm across -n $NS -o yaml --patch "$PATCH"
PATCH=$(kubectl get cm across -n $NS -o yaml | yq '.data.r13 |= (. | fromjson | .topology.links += [{"endpoints": ["macvlan:net1008", "r13:eth3"]}] | tojson)'); kubectl patch cm across -n $NS -o yaml --patch "$PATCH"
...
PATCH=$(kubectl get cm across -n $NS -o yaml | yq '.data.rupf |= (. | fromjson | .topology.links += [{"endpoints": ["macvlan:net2002", "rupf:eth2"]}] | tojson)'); kubectl patch cm across -n $NS -o yaml --patch "$PATCH"
...
kubectl rollout restart deployment r11 -n $NS
kubectl rollout restart deployment r12 -n $NS
kubectl rollout restart deployment r13 -n $NS
