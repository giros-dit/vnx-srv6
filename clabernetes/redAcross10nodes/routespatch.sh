...
kubectl exec deploy/ru -- docker exec ru sh -c 'echo "104 tunel4" >> /etc/iproute2/rt_tables'
kubectl exec deploy/ru -- docker exec ru sh -c 'echo "105 tunel5" >> /etc/iproute2/rt_tables'
kubectl exec deploy/ru -- docker exec ru sh -c 'echo "106 tunel6" >> /etc/iproute2/rt_tables'
...
kubectl exec deploy/ru -- docker exec ru ip -6 rule add to fd00:0:3::2 lookup tunel4
kubectl exec deploy/ru -- docker exec ru ip -6 rule add to fd00:0:3::3 lookup tunel5
kubectl exec deploy/ru -- docker exec ru ip -6 rule add to fd00:0:3::4 lookup tunel6