...
def createupf(vlan, podsdict, routers, method):
    ...
    podname = podsdict['r13']
    
    comandoupf = f"kubectl exec -n c9s-srv6 {podname} -- docker exec r13 ip -6 route {method} {ipgNB} encap seg6 mode encap segs {','.join(segments)} dev eth3.{VLAN}"
    
    return comandoupf

def creategnb(VLAN, podsdict, routers ,method):
    ...
    podname = podsdict[str(f"r{rp}")]

    comandognb = f"kubectl exec -n c9s-srv6 {podname} -- docker exec r{rp} ip -6 route {method} {ipUPF} encap seg6 mode encap segs {','.join(segments)} dev eth3.{VLAN}"
    return comandognb
...
def get_running_pods():
    namespace = "c9s-srv6"
    prefixes = ["r11", "r12", "r13"]
    pods_dict = {prefix: [] for prefix in prefixes}

    try:
        # Ejecuta el comando para obtener los pods
        command = (
            f"kubectl get pods -n {namespace} --field-selector=status.phase=Running --no-headers -o custom-columns=:metadata.name"
        )
        result = os.popen(command).read()

        # Procesa la salida del comando
        pods = result.strip().split("\n")
        for pod in pods:
            for prefix in prefixes:
                if pod.startswith(prefix):
                    pods_dict[prefix].append(pod)
...
def main():
    ...
    for vlan, routers in conf.items():
        ...
        if vlan not in conf_old or conf_old[vlan] != routers:
            ...
            commandr13 = createupf(VLAN, podsdict, routersupfgnb, action)
            commandgnb = creategnb(VLAN, podsdict, routersgnbupf, action)
