...
def createupf(vlan, podsdict, routers, method):
    ...
    comandoupf = f"kubectl exec -n {ns} {podname} -- docker exec r13 ip -6 route {method} {ipgNB} encap seg6 mode encap segs {','.join(segments)} dev eth3.{VLAN}"
...
def creategnb(VLAN, podsdict, routers ,method):
    ...
    comandognb = f"kubectl exec -n {ns} {podname} -- docker exec r{rp} ip -6 route {method} {ipUPF} encap seg6 mode encap segs {','.join(segments)} dev eth3.{VLAN}"
    return comandognb
...
def get_running_pods():
    ...
    try:
        ...
        command = (
            f"kubectl get pods -n {ns} --field-selector=status.phase=Running --no-headers -o custom-columns=:metadata.name"
        )
